...
Rcpp::List shrink_est(int lb, int m, int n, Rcpp::List X, Rcpp::List Y,
                     arma::vec beta, arma::mat mcov, arma::uvec pos,
                     arma::mat v) {
    double b_bar2 = 0;
    for (int i = 0; i < n; i++) {
        ...
        arma::mat vi = ui * ui.t();
        // scaled it
        double b_bar2i = pow(arma::norm(vi - v, "fro"), 2.0);
        b_bar2 += b_bar2i;
    }
    arma::mat I;
    I.eye(size(v));
    b_bar2 = b_bar2/n/n;
    double m_n = arma::trace(v * I);
